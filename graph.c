...
void reSortStartVertexIndex(SequenceType vertices[], stringNames temp, int numVertices, int *startingIndexVertex) {
  int i;
...
void printVertexDegrees(FILE **fp, SequenceType vertices[], int numVertices) {
  ...
  *fp = fopen("TRAVERSALS.txt", "w");
  ...
  padding = longestString(vertices, numVertices) + 3; 
